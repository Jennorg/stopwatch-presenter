...
class ControllerWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.time_label.setStyleSheet("font-size: 70px; font-family: Consolas; font-weight: bold;")
    ...
    def launch_and_start(self):
        """Proyecta la ventana y auto-inicia si es la pantalla principal."""
        ...
        # Si el reloj no está corriendo, aplicar valores de los inputs e iniciar
        if not self.logic.is_running and self.logic.elapsed_before_pause == 0:
            self.apply_inputs_to_logic()
            
        if target_screen == QGuiApplication.primaryScreen() and not self.logic.is_running:
    ...
    def handle_play_pause(self):
        ...
        # Si es un inicio "desde cero", leer los valores de las cajas de texto
        if not self.logic.is_running and self.logic.elapsed_before_pause == 0:
            self.apply_inputs_to_logic()
    ...
    def update_displays(self):
        ...
        if self.presenter:
            self.presenter.update_display(t, color, blink)
